---
title: 配置驱动按钮列表：函数返回的 loading 为什么还是响应式的
date: 2026-09-17
categories: [Vue]
tags: [Vue, 响应式原理, 渲染机制, 性能优化]
description: 从依赖收集机制讲清 buttonProps 函数写法为何能保持 loading 响应式，并分析 loading 变化时的渲染粒度，以及 index 作为 key 在静态列表中的影响。
---
