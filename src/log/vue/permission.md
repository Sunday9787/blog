...
logIn(path2)->logOut
logIn(path1,bottom)->permission->route->view->logOut->end
```
...
写个递归组件渲染即可，是否渲染菜单 则需要通过 `hasPermission` 方法判断 当前 route.name (权限key)
...
路由钩子：实现过程不复杂，主要在 router 拦截那边稍微绕了点
